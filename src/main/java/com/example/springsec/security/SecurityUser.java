...
public class SecurityUser implements UserDetails {
    ...
    public static UserDetails fromUser(User user) {
        ...
        user.getAccessRoles().stream().map(accessRole -> {
            accessRole.getPrivileges().stream().map(privilege -> {
                simpleGrantedAuthorities.add(new SimpleGrantedAuthority(privilege.getCode()));
                return null;
            });
            return null;
        });
